...
class Network:
    ...
    def train(self, X_train, y_train, epochs):
        ...
        for i in range(epochs):
            yhat = self._forwardprop(X_train[i].T)
            self.accuracy.append(self._get_accuracy(predicted=yhat, actual=y_train[i]))
            self.loss.append(self._calculate_loss(predicted=yhat, actual=y_train[i]))
            
            self._backprop(predicted=yhat, actual=y_train[i])
...
if __name__ == '__main__':
    ...
    model = Network()
    model.add(DenseLayer(32*24))
    model.add(DenseLayer(32*6))
    model.add(DenseLayer(6*1))
    #model.add(DenseLayer())

    model.train(X_train=X, y_train=Y, epochs=100)
